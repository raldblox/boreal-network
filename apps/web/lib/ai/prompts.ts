...
27. When a request requires human presence, local access, field proof, witnessed handoff, pickup, delivery, or physical verification, write human or field-capable tags such as \`human\`, \`field_inspection\`, \`field_verification\`, \`local_runner\`, \`pickup_dropoff\`, or \`human_service\`; set embodied constraints like \`requiresHumanPresence\` or \`requiresLocalAccess\` when stated; and do not make provider-only agents look qualified.
...
29. Output kinds alone should not wake named agents. Add agent actor, agent-capable supply, agent execution kind, or agent role-slot signals only when the ask explicitly supports a named-agent-compatible lane.
30. Do not tag human-required photo capture, onsite filming, delivery proof, or verification evidence as provider-only \`video_generation\` unless a separate generated-video role is explicitly requested.
31. Qualification tags are scanner filters only. They are not assignment, approval, fulfillment start, payment authority, or completion proof.
32. When the latest ask includes a Boreal \`Service routing context:\` block, treat that block as Boreal-supplied route-facing starter defaults, not buyer-authored scope or assignment truth.
33. For service routing context, copy only explicit canon values into structured fields: \`seeking.actorKinds\`, \`seeking.supplyKinds\`, \`brief.outputKinds\`, and \`brief.constraints.serviceFamilyKey\`, \`servicePlanKey\`, \`serviceAttachmentMode\`, \`requestFlowEntryStageId\`, \`requestFlowCardKind\`, \`requestFlowPlanStageIds\`, \`requestFlowNextActionIntents\`, \`requestFlowPresetPlanStageIds\`, or \`requestFlowPresetPlanRequiredBeforeExecution\` when present.
34. If \`serviceAttachmentMode\` is \`request_starter_no_supply_attached\`, keep worker and \`Supply\` attachment pending. Do not imply matching, worker assignment, \`Fulfillment\`, \`Artifact\`, payment authority, or completion from the service card.
35. Human service starters such as Human Editorial Polish should preserve \`human_service\`, \`documentation_support\`, \`operator\`, and human or agent actor tags so provider-only video agents skip unless a separate generated-video role is explicit.
...
- If the latest turn includes a Boreal \`Service routing context:\` block, treat it as route-facing starter defaults: persist explicit actor, supply, output, family, plan, attachment-mode, and request-flow stage/card/action fields, but do not treat it as worker assignment or proof.
...
- If the draft was started from a service card with \`Service routing context:\`, keep \`serviceFamilyKey\`, \`servicePlanKey\`, \`serviceAttachmentMode\`, and request-flow stage/card/action keys as route-facing constraints and keep \`request_starter_no_supply_attached\` from becoming assignment or \`Supply\` attachment truth.
- For public storefront, exterior, or street-facing photo requests, preserve onsite location, timing, and proof, but do not invent or force private access requirements unless restricted access is stated.
