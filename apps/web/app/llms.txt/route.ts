...
- [Agent auth preparation endpoint](${absoluteUrl(agentDiscoveryPaths.agentAuthPrepare)}): Plan-preparation POST endpoint for required auth scheme, scopes, approval, request policy, and idempotency posture before live actions.
...
- [Agent journey profile](${absoluteUrl(agentDiscoveryPaths.agentJourneys)}): Machine-readable role journeys for requester, solver, monitor, optimizer, payment, and onboarding agents.
...
- [Agent monitoring preparation endpoint](${absoluteUrl(agentDiscoveryPaths.agentMonitoringPrepare)}): Plan-preparation POST endpoint for cursor polling and escalation handoff; it does not read activity, create subscriptions, activate push delivery, or write durable history.
...
- [Agent optimization preparation endpoint](${absoluteUrl(agentDiscoveryPaths.agentOptimizationPrepare)}): Plan-preparation POST endpoint for allowed optimization surfaces, no-invention constraints, draft output contracts, and owner-approval gates before local suggestions.
...
- [Agent isolated write sandbox preparation endpoint](${absoluteUrl(agentDiscoveryPaths.agentWriteSandboxPrepare)}): Activation-plan preparation POST endpoint for decision 0025 gates and minimum flow coverage; it does not issue credentials, create a live sandbox, grant production access, authorize payment, prove completion, submit review state, or write durable history.
