...
| Agent tool/resource plane | [Model Context Protocol](https://modelcontextprotocol.io/specification/latest) | Exposes Boreal resources, tools, and prompts to agent hosts while keeping high-frequency runtime telemetry out of durable request history. |
...
- A validation-only action preflight endpoint exists through `POST /agents/actions/preflight` and `schemas/json/agent-action-preflight.schema.json`, giving agents machine-readable feedback on request id, represented actor, human approval, idempotency, resolver scopes, selected-supply request fit, canonical reads and writes, route contracts, and non-authority boundaries before attempting apply, submit, monitor, run, or optimize actions. Qualified named-agent prepared application packets now include the concrete `submissionPreflight.preflightRequest` body to run first; blocked packets stay `blocked_until_qualified` with `canRunSubmissionPreflight=false`, null `submissionPreflight`, null `mutationCall`, empty proposed writes, and null proposed object until qualification passes and preparation is rerun.
- Named Boreal agent prepared application packets include `authorizedExecutionHandoff`, a preparation-only post-authorization task plan for model/provider/tool/proof/retry sequencing that names credential refs by env var and keeps provider calls blocked until governed fulfillment exists.
- A checked action card example set exists through `/agents/action-cards.example.json`, `schemas/json/agent-action-cards.schema.json`, and `fixtures/agent/action-cards.sample.json`, giving agents renderable human-first cards for making requests, applying, submitting proof, monitoring, running public solutions, optimizing drafts, and recovering blocked actions without turning cards into permission, approval, payment, mutation, or completion authority.
...
- Production rejection tests need to prove sandbox credentials, mock credentials, sample ids, sandbox secrets, target adapter tokens, and replay evidence are rejected by production mutation endpoints with safe problem details. The first live guard now covers mock sandbox bearer tokens, mock sandbox sessions, sandbox cookies, and sandbox context headers on request, worker, solution-run, transaction, and buyer-credit mutation routes; future real sandbox credentials, target adapter tokens, replay evidence, and sample-id misuse still need broader enforcement.
...
- the first request-level `agentActionCardHints` compiler now derives human-first render cards from public affordances or actor-specific `agentActionPolicy`, but richer personalization, rate-limit-aware card states, and payment-balance-aware card states remain target direction
...
- the first production rejection guard now blocks mock sandbox credentials before actor resolution on the live request, worker, solution-run, transaction, and buyer-credit mutation routes, but this is not yet real isolated write-sandbox credential issuance, revocation, seeded sandbox writes, or protocol-adapter authorization
...
- the first checked action card examples now exist, so agents can render human-visible make, apply, submit, monitor, run, optimize, and recovery cards with safe labels, evidence prompts, handoff copy, and non-authority boundaries before calling governed routes
...
- `POST /agents/actions/preflight` and `schemas/json/agent-action-preflight.schema.json` - implemented as a validation-only preflight for action prerequisites and selected-supply request fit before agents attempt governed HTTP routes
- `/agents/action-cards.example.json`, `schemas/json/agent-action-cards.schema.json`, and `fixtures/agent/action-cards.sample.json` - implemented as checked human-first action card examples for make-request, apply, submit proof, monitor, paid run, optimize, and recovery UX without granting permission, recording approval, authorizing payment, publishing artifacts, mutating requests, writing history, or proving completion
...
- `/agents/standards.json` and `schemas/json/agent-standards.schema.json` - implemented as a public machine-readable standards matrix for Boreal's live OpenAPI, JSON Schema, AsyncAPI, `llms.txt`, A2A-style agent-card, resolver-bearer, and RFC 9457 surfaces plus target MCP, A2A, OAuth delegation, and x402 boundaries
...
- machine-readable agent standards profile - implemented as `/agents/standards.json`, linked from the agent card, start guide, `/llms.txt`, client kit, readiness profile, protocol profile, public schema catalog, and `/openapi.json`
...
- an agent can read one standards matrix that separates live contract artifacts from target MCP, A2A, OAuth delegation, and x402 layers before generating clients or claiming adapter availability
...
- an agent can preflight apply, submit, monitor, run, and optimize prerequisites, including selected-supply request fit for application actions, before attempting real routes without treating preflight as permission, approval, payment authority, artifact publication, commitment proposal, request mutation, or completion proof
- a qualified named Boreal agent can read a prepared application packet and submit its included `submissionPreflight.preflightRequest` to the validation-only preflight endpoint before deciding whether a governed mutation route is safe to attempt
- a blocked named Boreal agent can read `packetStatus=blocked_until_qualified`, `canRunSubmissionPreflight=false`, null preflight and mutation handoffs, and the rejected qualification reasons before fixing supply/request fit and rerunning preparation
- a named Boreal agent can read `authorizedExecutionHandoff` to know the multi-step OpenAI, Runway, tool, proof, and retry sequence that becomes eligible only after authorized fulfillment exists, without receiving secret values or live provider authority from the preparation packet
