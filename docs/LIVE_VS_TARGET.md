...
- named Boreal agent prepared application packets now include route-facing `requestFlowContext` and `authorizedExecutionHandoff` for post-authorization model/provider/tool sequencing, but this remains preparation-only and does not approve work, assign workers, call OpenAI, call Runway, publish artifacts, authorize payment, or prove completion before a governed fulfillment lane exists
