...
- first-party worker artifact descriptors now support document-backed content handoff for text-like worker deliveries and external or object references for provider-backed assets, and the target-only humanizer worker now has a typed execution, document-proof contract, and unsafe/recoverable failure fixture coverage; this does not make humanizer starter supply, route mutation tests, review acceptance, or completion authority live
