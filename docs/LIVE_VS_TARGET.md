...
- one beta homepage candidate can render requests, services, campaigns, and reusable outcomes as UI-level Boreal work cards with visible request, plan, worker, funding, and outcome slots plus typed card taxonomy for canonical root, listing kind, worker attachment state, next boundary, in/out scope, shared request-flow `stageId`, `cardKind`, actor modes, authority boundary, and next action intents; service families and preset plans now carry typed request-flow entry, plan-stage, authority, action-intent, and pre-execution metadata; service prefill text exposes routing context and no-assignment boundaries; deterministic request briefing preserves those service and request-flow defaults as route-facing constraints; and service request starters keep worker or `Supply` attachment pending until checkout or request routing instead of pretending the listing assigned supply
...
- one schema-backed actor-neutral request-flow taxonomy profile now exists under `schemas/json/request-flow-stage-card-taxonomy.schema.json`, `fixtures/request/request-flow-stage-card-taxonomy.sample.json`, and `apps/web/lib/request-flow-taxonomy.ts`, defining closed stage ids, card kinds, actor modes, authority gates, next-stage references, and n8n sidecar constraints without activating live drag mutations or import/export routes
...
- one buyer-facing services surface can present first-party service families and preset plans with typed request defaults and request-flow metadata without replacing `Supply` or `Request` as canonical roots
...
- production mutation routes for request drafts, request routing, commitments, fulfillments, artifacts, solution runs, request transactions, and buyer-credit mutations now share a live sandbox-credential rejection guard and `pnpm web:test:agent-sandbox-production-boundary` verifies mock sandbox bearer tokens, mock sandbox sessions, sandbox cookies, and sandbox context headers return safe problem details with permission, durable-write, payment, and completion flags false before normal auth or write paths
...
- Boreal-managed `video-generation` and `humanizer` starter supplies exist as the first in-house worker-backed supply lanes; broader in-house worker matching, production sandbox write credentials, and protocol adapters remain target work
- one machine-readable in-house worker application profile now defines named agent templates, including `Mira` as the video-generation route template and `Tala` as the humanizer route template; public agent discovery lists those named templates with the shared `boreal_named_agent_v1` framework, promotion gates, boilerplate refs, and preparation-only route mode; a shared registry builder validates stable route, unique name, framework settings, live or target promotion gates, supply binding, model binding, task pipeline, boilerplate refs, and canonical write requirements; and one live preparation-only per-agent route surface can read those templates, scan caller-supplied request summaries, run bounded public open-request scans for wake or skip decisions, and prepare non-mutating application packets with route-facing `requestFlowContext` plus governed mutation-call sketches for existing commitment or owner-private fulfillment routes
- first-party worker artifact descriptors now support document-backed content handoff for text-like worker deliveries and external or object references for provider-backed assets, and the live humanizer worker now has a typed execution, document-proof contract, supply factory, route-level preparation tests, and unsafe/recoverable failure fixture coverage; this does not make review acceptance or completion authority live
- the reusable public request board now renders read-only worker readiness from already-loaded public-safe request projection fields, including human-required, human-can-apply, Mira can-prepare, Tala can-prepare, skip, and target-only states without assignment, private fetches, provider calls, payment authority, or durable writes; actionable human lanes carry an `apply_to_request` preflight handoff for later governed `Commitment` proposals
- named-agent and public-board scanner gates now use shared request qualification over actor, supply, output, execution, `workerEligibility`, embodied, and verification-plan fields so provider-only workers skip human, local-access, field-inspection, pickup or handoff, physical-proof, and location-proof requests even when those requests also contain video-like wording, unless the planner marks an explicit supporting agent role
- named Boreal agent prepared application packets now include route-facing `requestFlowContext`, `/agents/actions/preflight` validates known request-flow values and non-authority flags when that context is present, and packets include `authorizedExecutionHandoff` for post-authorization model/provider/tool sequencing; this remains preparation-only and does not approve work, assign workers, call OpenAI, call Runway, publish artifacts, authorize payment, or prove completion before a governed fulfillment lane exists
