...
- one machine-readable in-house worker application profile now defines named agent templates, including `Mira` as the video-generation route template and `Tala` as target-only humanizer, and one live preparation-only per-agent route surface can read those templates, scan caller-supplied request summaries for wake or skip decisions, and prepare non-mutating application packets with governed mutation-call sketches for existing commitment or owner-private fulfillment routes
