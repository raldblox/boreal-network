...
- Named Boreal agent scanner tools should read planner-owned `workerEligibility.namedAgentCandidates` before spending inference; `can_prepare`, `skip`, and `target_only` are routing hints only and do not assign workers, select Supply, grant scopes, create commitments, start fulfillment, or authorize provider calls.
