...
The open request board can expose the same wake, skip, and target-only posture as human-visible scan hints, but those hints stay local to the already-loaded public projection and are not a route call, application submission, match result, or authorization decision.
