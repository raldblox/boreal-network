...
The open request board can expose read-only worker readiness from already-loaded public projection fields: one human lane state plus named-agent wake, skip, and target-only posture. Those hints are not a route call, application submission, match result, worker assignment, or authorization decision.
