...
Prepared application packets must carry `packetStatus`, `qualificationGate`, and lane-specific `requiredNextAction` so a direct `prepare_application` response that failed qualification is visibly `blocked_until_qualified` before agents run preflight or attempt a sketched mutation route.
