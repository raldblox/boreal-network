...
Named-agent qualification must honor public projection fields such as `seeking.actorKinds`, `seeking.supplyKinds`, `brief.outputKinds`, `brief.constraints`, `derived.executionKind`, `derived.executionProfile`, `derived.embodiedConstraintSet`, and `derived.verificationPlan` so provider-only agents skip human-required, local-access, field-inspection, pickup or handoff, physical-proof, or location-proof work before preparing an application.
