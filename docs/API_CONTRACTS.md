...
Named-agent qualification must honor public projection fields such as `seeking.actorKinds`, `seeking.supplyKinds`, `brief.constraints`, and execution-kind hints so provider-only agents skip human-required or local-access work before preparing an application.
