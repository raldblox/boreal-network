...
The workroom selected-worker start action may submit that same route with `initialStatus=planned`, selected published Boreal-managed `supplyId`, and `ownerPrivateDirectApproval.workerKey`; this creates the execution boundary and seeded steps only, not provider execution, artifact proof, owner acceptance, payment authority, or completion.
