...
- `GET /api/boreal-agents/{agentKey}` and `POST /api/boreal-agents/{agentKey}` route tests keep the named-agent route preparation-only: template reads and application packets may be prepared, but `Request`, `Commitment`, `Fulfillment`, `FulfillmentStep`, `Artifact`, `Transaction`, provider calls, and `RequestEvent` writes must stay behind separate authorized mutation routes
