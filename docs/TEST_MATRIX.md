...
- planner-derived `Request.derived.workerEligibility.namedAgentCandidates` must mark compatible live named agents as `can_prepare`, incompatible agents as `skip`, and blocked future agents as `target_only` without creating assignment, Supply attachment, Commitment, Fulfillment, provider calls, payment authority, RequestEvent history, or completion proof
