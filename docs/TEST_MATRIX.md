...
- the named-agent template builder rejects duplicate names or routes, unstable agent keys, missing required task-pipeline steps, missing model or supply bindings, and unknown canonical writes before a new agent can be exposed through discovery or `/api/boreal-agents/{agentKey}`
