...
- `video-generation` remains the only live starter worker until another worker such as humanizer has a real supply factory, failure fixtures, and route-level mutation tests
...
- named-agent promotion gates must make `Mira` `live_backed` with no open blockers and `Tala` `target_blocked` with blockers for missing humanizer supply factory, failure fixtures, and mutating route tests
