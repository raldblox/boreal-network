...
- direct named-agent `prepare_application` responses must expose `packetStatus`, `qualificationGate`, and lane-specific `requiredNextAction`, keeping disqualified packets `blocked_until_qualified` with blocked reasons, `canRunSubmissionPreflight=false`, null `submissionPreflight`, null `mutationCall`, empty proposed writes, and null proposed object before any preflight or sketched mutation attempt
- direct named-agent `prepare_application` responses must block when `Request.derived.workerEligibility.namedAgentCandidates` marks the current named agent as `skip` or `target_only`, even if broader request fingerprints otherwise look compatible
...
- planner-derived `Request.derived.workerEligibility.namedAgentCandidates` must mark compatible live named agents as `can_prepare`, incompatible agents as `skip`, and blocked future agents as `target_only`; board readiness and scan output must surface those hints as `plannerCandidate` display context, and scan output must short-circuit `skip` or `target_only` hints before preparation without creating assignment, Supply attachment, Commitment, Fulfillment, provider calls, payment authority, RequestEvent history, or completion proof
- first-party worker starter catalog contract tests must keep each starter entry aligned with its live worker `Supply.capability`, provider ref, named-agent template, scanner qualification tags, wake signals, and non-authority flags before starter lanes are shown as reusable frontpage supply affordances
...
- planner-derived `workerEligibility` must classify human-first skip, human-first agent-support, raw-not-planned, no-agent-signal, wake-named-agents, and per-agent `namedAgentCandidates` cases from canonical fingerprints without assigning supply, creating commitments, starting fulfillment, calling providers, authorizing payment, writing events, or proving completion
...
- owner-approved private readiness tests render direct `Fulfillment` preparation only when selected supply, trusted-worker auto-approval, eligible status, and matching worker key gates are present; missing gates stay `skip` and do not fall back to public `Commitment` application hints
