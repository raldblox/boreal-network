...
- `/agents/sandbox.md` and `/agents/sandbox.json` render a contract-only sandbox with deterministic mock identities, sample IDs, payloads, replay scenarios, and canonical boundaries
...
- `pnpm contracts:agent-sandbox` validates sandbox fixture coverage for inspect, make-request, apply, submit, monitor, run, signed webhook, optimize, replay, paid-run shape, error examples, human handoff packet examples, production access packet examples, protocol adapter samples, and idempotent recovery flows
...
- `apps/web/tests/contracts/agent-discovery.test.ts` covers the public agent card, start guide, action playbook, client-kit manifest, journey profile, sandbox guide, sandbox manifest, action catalog, discovery index, allowlisted contract assets, and absence of obvious secret material in exported contracts
