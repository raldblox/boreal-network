...
- every prepared application packet must carry route-facing `requestFlowContext` when request constraints provide stage, card, action, preset-plan, or pre-execution metadata; that context is guidance for the next boundary only and is not approval, assignment, or mutation authority
