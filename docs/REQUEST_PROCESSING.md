...
It may persist explicit canon values from that block into `seeking.actorKinds`, `seeking.supplyKinds`, `brief.outputKinds`, and route-facing constraints such as `serviceFamilyKey`, `servicePlanKey`, `serviceAttachmentMode`, `requestFlowEntryStageId`, `requestFlowCardKind`, `requestFlowPlanStageIds`, `requestFlowNextActionIntents`, `requestFlowPresetPlanStageIds`, and `requestFlowPresetPlanRequiredBeforeExecution`.
...
- block direct named-agent `prepare_application` when the current agent's planner candidate is `skip` or `target_only`, keeping the blocked packet null for preflight, mutation call, proposed canonical writes, and proposed object
...
- keep first-party worker starter catalog entries typed to the same `Supply.capability`, named-agent binding, scanner qualification tags, wake signals, and non-authority flags as the live worker definition and named-agent template, so enabling a starter lane cannot drift from request planner fingerprints
...
- every prepared application packet must carry route-facing `requestFlowContext` when request constraints provide stage, card, action, preset-plan, or pre-execution metadata; that context is guidance for the next boundary only and is not approval, assignment, or mutation authority; action preflight should reject unknown request-flow values or missing context non-authority flags before any mutation route is attempted
- blocked prepared application packets must keep `submissionPreflight` and `mutationCall` null, `proposedCanonicalWrites` empty, and `proposedObject` null until qualification passes and the agent reruns preparation
- qualified prepared application packets must carry a `submissionPreflight` handoff requiring `/agents/actions/preflight`, `apply_to_request` for public or cross-actor `Commitment` application or `create_owner_private_fulfillment` for owner-private direct `Fulfillment`, represented actor evidence, idempotency, visible `requestFit` selected-supply status and capability fingerprints, route-facing `requestFlowContext`, a concrete `preflightRequest` body, request detail, `agentActionPolicy`, selected `Supply`, active refs, funding, proof, and lane-specific route-policy rechecks before any sketched mutation route is attempted
...
- Flow-card drag actions are explicit projection taxonomy: `Request` cards create or refine request-owned plan phases, `phase` or `stage` cards prepare human or agent worker applications, `worker` cards review delivery paths, and `delivery` or `step` cards inspect proof. Current `RequestFlowNodeDescriptor` projections carry `stageId`, `cardKind`, `actorModes`, `authorityBoundary`, done/not-done scope, and `nextActionIntents`; action options must stop instead of guessing when the visible drag action is not listed in the node taxonomy. The drag gesture itself is not a mutation, assignment, `Commitment`, `Fulfillment`, `Artifact`, or completion claim.
...
- `RequestDerived.workerEligibility` is scanner guidance only. It may say whether humans are required, named agents should wake, provider-only agents should skip, explicit human-first agent support is present, which canonical fingerprints caused that decision, and which named agents have `can_prepare`, `skip`, or `target_only` planner hints; it must not assign `Supply`, create `Commitment`, start `Fulfillment`, call providers, authorize payment, write `RequestEvent`, or prove completion.
- `RequestDerived.workerEligibility.wakeSignals` should not be derived from `brief.outputKinds` alone. Output kinds such as `draft`, `file`, `handoff_doc`, `media`, or `video` support an agent wake decision only after the plan also has an agent actor, agent-capable supply, agent execution kind, or agent role slot.
