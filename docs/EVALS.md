...
- `pnpm --filter @boreal/web exec tsx tests/contracts/showcase-request-catalog.test.ts`
  Validates beta work-card taxonomy for service request starters, open request listings, reuse-ready requests, canonical `Request` root, worker attachment state, next boundary, explicit in/out scope, and service-family request defaults such as Human Editorial Polish projecting to `human_service` plus `documentation_support` instead of generic provider supply.
