...
Verify that disqualified direct `prepare_application` responses expose `packetStatus=blocked_until_qualified`, blocked reasons, and `canRunSubmissionPreflight=false` before any action preflight or sketched mutation route is attempted.
