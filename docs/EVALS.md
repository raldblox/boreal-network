...
Verify that a funded private workroom can create a `planned` selected-worker `Fulfillment` only from a pinned published Boreal-managed `Supply` with matching owner-private approval evidence, while provider execution, artifact publication, review acceptance, and completion remain blocked.
Verify that starting that selected first-party worker lane uses the existing `Fulfillment`, bridges `planned -> ready -> active` before provider execution, preserves selected `Supply` attachment, and still cannot claim artifact proof, owner acceptance, payment authority, or completion.
...
- `pnpm web:test:owner-private-worker-fulfillment-start`
  Validates the workroom selected-worker lane helper: it permits only owned private compatible requests with a pinned published Boreal-managed Supply, builds a planned owner-private fulfillment body with matching worker key approval evidence, and keeps provider calls false before execution.
