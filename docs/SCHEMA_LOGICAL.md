...
- worker-routing derived state includes `workerEligibility`, a rebuildable scanner guidance object with human-required, named-agent wake, explicit human-first agent support, provider-skip, canonical fingerprint, wake-signal, skip-reason, per-agent `namedAgentCandidates`, and non-authority fields
