...
| [0027-in-house-boreal-agents-scan-and-apply-as-workers.md](0027-in-house-boreal-agents-scan-and-apply-as-workers.md) | `accepted` | In-house worker scanner and auto-approval policy are aligned; broader application routes and fixtures remain target. | In-house Boreal agents scan read-only and apply through `Commitment` or owner-private direct `Fulfillment`; prompt-only assets are support assets, not frontpage `Supply`. | Review before changing internal worker matching, auto-approval, worker applications, or adding humanizer/service-backed starter supply. |
