...
| [0026-actor-neutral-request-flow-stage-card-taxonomy.md](0026-actor-neutral-request-flow-stage-card-taxonomy.md) | `accepted` | Strategy taxonomy now has schema, fixture, and TypeScript profile coverage; live mutations and adapters remain target. | Shared request-flow stages and cards must be actor-neutral across humans, agents, systems, and adapter mappings. | Review before changing request-flow card schemas, changing `agentActionCardHints`, or adding n8n import/export routes. |
| [0027-in-house-boreal-agents-scan-and-apply-as-workers.md](0027-in-house-boreal-agents-scan-and-apply-as-workers.md) | `accepted` | In-house worker scanner and auto-approval policy are aligned; `video-generation` and `humanizer` have live preparation templates. | In-house Boreal agents scan read-only and apply through `Commitment` or owner-private direct `Fulfillment`; prompt-only assets are support assets, not frontpage `Supply`. | Review before changing internal worker matching, auto-approval, worker applications, or adding more service-backed starter supply. |
